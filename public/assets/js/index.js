...
fetch('./songs.json')
	...
	.then((songs) => {
		...
		navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then((stream) => {
			...
			let maxAudioFrequency,
				ipRangePrefix,
				channels,
				channelValuesSum,
				channelValuesCount,
				lastCalculatedChannel,
				percentFactor,
			...
			fetch('./config.json')
				...
				.then((config) => {
					...
					function getColors(channelId, deviceId) {
						...
						if (
							deviceColors === undefined ||
							deviceColors[colorId] === undefined ||
							deviceColors[colorId].length === 0
						) {
							return false;
						}
						...
						throttle(
							function (colorId) {
								lastColors[colorId] =
									lastColors[colorId] === undefined ? -1 : parseInt(lastColors[colorId]);
					...
					function drawLightShow(channel, audioValue, bar) {
						...
						if (channel > lastCalculatedChannel || bar === maxAudioFrequency) {
							...
							const lastFiveHistoricalStatus = statusHistory[lastCalculatedChannel].slice(
								-numberOflastTurnOns,
							);
							...
							if (lastFiveHistoricalStatus.length > 0) {
								const lastTurnOns =
									lastFiveHistoricalStatus.reduce(
							...
							fillHTMLDebugData(
								lastCalculatedChannel,
								`${Math.floor(channelValuesSum / channelValuesCount)} - ${channelThreshold}`,
							);
